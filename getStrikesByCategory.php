...
// Build the SQL query
if ($update) {
  $sql = "SELECT catDef.name AS category_name, subCatDef.name AS subcategory_name, subCatTest.current AS skill_value, skills.value AS skill_min_value, subCatTest.target AS skill_target_value
  ...
  LEFT JOIN tbl_210_subcategories_test AS subCatTest ON subCatTest.user_id = skills.user_id AND subCatTest.category_id = skills.category_id AND subCatTest.subcategory_id = skills.subcategory_id AND subCatTest.goal_id = $goal_id
